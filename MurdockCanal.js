...
const cached = JSON.parse(fs.readFileSync(__dirname + "/cache.json", "utf8"));
let photos = [];

fs.readdir(__dirname + "/photos/", (err, files) => photos = files);
...
function ParseListings(html) {
    ...
    $(".property").each((index, ele) => {
        ...
        if (!photos.includes(newProperty.address + ".jpg")) {
            DownloadImage($(ele).find("img").attr("src"), __dirname + "/Photos/" + newProperty.address + ".jpg");
        }
...
function LoadTrail() {
    return JSON.parse(fs.readFileSync("C:/users/kevin/documents/programming projects/murdockcanal/trail.json", "utf8"));
}
...
module.exports = {
    GetListings: function() {
        return new Promise((resolve, reject) => {
            if (Date.now() - JSON.parse(fs.readFileSync(__dirname + "/config.json", "utf8")).last_pulled > 24 * 60 * 60 * 1000) {
                GetListings(250000, 450000).then(properties => {
                    ...
                    Promise.all(promises).then(result => {
                        fs.writeFile(__dirname + "/cache.json", JSON.stringify(cached), () => { });
                        fs.writeFile(__dirname + "/properties.json", JSON.stringify(properties), () => { });
                        fs.writeFile(__dirname + "/config.json", JSON.stringify({ "last_pulled": Date.now() }), () => { });
                        resolve(JSON.stringify(properties));
                    });
                });
            }
            else {
                resolve(fs.readFileSync(__dirname + "/properties.json", "utf8"));
            }
        });
    },
    ...
    GetTrail: function(){
        return new Promise((resolve, reject)=>{
            resolve(fs.readFileSync(__dirname + "/trail.json", "utf8"));
        });   
    } 
}